...
Функция ПолучитьСообщения(Подписчик) Экспорт 
	...
	//РШ++
	Если ОтветЗапроса.Кодировка = Неопределено Тогда
	
		ОтветЗапроса.Кодировка = "utf-8";	
...
Процедура ПолучитьНаСервереИзКафки() Экспорт
	...
	Кафка.Подписаться(Подписчик, "test_topic");	
	Сообщения = Кафка.ПолучитьСообщения(Подписчик);
	Для каждого Сообщение из Сообщения Цикл 
		ТелоСообщения = Сообщение.Получить("value");
		Ключ = Сообщение.Получить("key");
		
		Если ЗначениеЗаполнено(ТелоСообщения) Тогда
		
			Процесс = СоздатьБизнесПроцесс(ТелоСообщения);	
		    ЗаписатьВЖурналКафка(Ключ,ТелоСообщения,Процесс);
		КонецЕсли;
	...
	ОценкаПроизводительности.ЗакончитьЗамерВремени("ПриемКафка",ВремяНачала);
КонецПроцедуры

Процедура ЗаписатьВЖурналКафка(Ключ,ТелоСообщения,Процесс)

	Запись = РегистрыСведений.ЖурналСообщенийКафка.СоздатьМенеджерЗаписи();
	Запись.Период = ТекущаяДата();
	Запись.Ключ = Ключ;
	Запись.Сообщение = ТелоСообщения;
