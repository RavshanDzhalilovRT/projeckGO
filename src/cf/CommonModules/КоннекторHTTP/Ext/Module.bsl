...
Функция JsonВОбъект(Json, Кодировка = "utf-8", ПараметрыПреобразования = Неопределено) Экспорт
	...
	Если ТипЗнч(Json) = Тип("ДвоичныеДанные") Тогда 
		...
		Json.Записать(ИмяВременногоФайла);
		текст = новый ЧтениеТекста;
		Текст.Открыть(ИмяВременногоФайла,Кодировка); 
		ТесткСтроки = Текст.Прочитать();
		Текст.Закрыть();
		ЧтениеJSON.ОткрытьФайл(ИмяВременногоФайла, Кодировка);
